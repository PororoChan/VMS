        ...
        $datatables->updateRow(function ($db, $nomor) {
            return [
                ...
                " <a class='btn btn-sm btn-warning eee' href='" . base_url('usergroup/EditViews/' . $db->groupid . '') . "'><i class='fas fa-pencil-alt'></i></a> " .
                    " <button type='button' class='btn btn-sm btn-danger hhh' onclick=\"deleteGlobal('VMS', 'Are you sure want to delete this usergroup ?', 'modal-md', '" . $db->groupid . "', '" . base_url('usergroup/deleteData') . "', '" . base_url('/usergroup') . "', 'Delete')\"><i class='far fa-trash-alt'></i></button>",
            ];
        });
        $datatables->toJson();
    }
